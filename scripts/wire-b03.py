...
s = p.read_text()
old = "import { DRINK_MASTER_EXPANSION_B02, DRINK_MASTER_EXPANSION_B02_SEED_ROWS, DRINK_MASTER_EXPANSION_B02_ALIAS_ENTRIES, DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B02_EVALUATED_AT } from './drink-master-expansion-b02.mjs';"
...
old = "const V19_BY_KEY = new Map([...JP_RARITY_V19_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B01_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B02_SEED_ROWS].map((row) => [normalizeDrinkV19Key(row[0]), row]));"
new = "const V19_BY_KEY = new Map([...JP_RARITY_V19_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B01_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B02_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B03_SEED_ROWS].map((row) => [normalizeDrinkV19Key(row[0]), row]));"
assert old in s
s = s.replace(old, new, 1)
old = "for (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B02_ALIAS_ENTRIES) {\n  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));\n}\nconst ACCEPTED_EVIDENCE_VERSIONS = new Set([EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B01_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION]);"
new = "for (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B02_ALIAS_ENTRIES) {\n  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));\n}\nfor (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B03_ALIAS_ENTRIES) {\n  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));\n}\nconst ACCEPTED_EVIDENCE_VERSIONS = new Set([EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B01_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION]);"
assert old in s
s = s.replace(old, new, 1)
...
'''
s = s.replace(marker, '\n' + block + marker, 1)
old = "  if (!(await hasExpansionB02Seed(env))) await seedExpansionB02(env);"
new = old + "\n  if (!(await hasExpansionB03Seed(env))) await seedExpansionB03(env);"
assert old in s
s = s.replace(old, new, 1)
old = "export { JP_RARITY_V19_SEED_ROWS as V19_ROWS, rarityLabel, rarityReason, canonicalLookupKey, enrichV19Response, hasCurrentV19Seed, hasExpansionB01Seed, hasExpansionB02Seed };"
new = "export { JP_RARITY_V19_SEED_ROWS as V19_ROWS, rarityLabel, rarityReason, canonicalLookupKey, enrichV19Response, hasCurrentV19Seed, hasExpansionB01Seed, hasExpansionB02Seed, hasExpansionB03Seed };"
assert old in s
s = s.replace(old, new, 1)
p.write_text(s)
