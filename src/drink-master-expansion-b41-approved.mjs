...
import { DRINK_MASTER_EXPANSION_B38 } from './drink-master-expansion-b38-approved.mjs';
import { DRINK_MASTER_EXPANSION_B39 } from './drink-master-expansion-b39-approved.mjs';
import { DRINK_MASTER_EXPANSION_B40,DRINK_MASTER_EXPANSION_B40_EXCLUDED } from './drink-master-expansion-b40-approved.mjs';
import { DRINK_MASTER_EXPANSION_B40_CANDIDATES } from './drink-master-expansion-b40.mjs';
import { DRINK_MASTER_EXPANSION_B41_MODERN_CANDIDATES,DRINK_MASTER_EXPANSION_B41_EVIDENCE_VERSION,DRINK_MASTER_EXPANSION_B41_EVALUATED_AT } from './drink-master-expansion-b41.mjs';
export { DRINK_MASTER_EXPANSION_B41_EVIDENCE_VERSION,DRINK_MASTER_EXPANSION_B41_EVALUATED_AT };
const PRIOR=[DRINK_MASTER_EXPANSION_B01,DRINK_MASTER_EXPANSION_B02,DRINK_MASTER_EXPANSION_B03,DRINK_MASTER_EXPANSION_B04,DRINK_MASTER_EXPANSION_B05,DRINK_MASTER_EXPANSION_B06,DRINK_MASTER_EXPANSION_B07,DRINK_MASTER_EXPANSION_B08,DRINK_MASTER_EXPANSION_B09,DRINK_MASTER_EXPANSION_B10,DRINK_MASTER_EXPANSION_B11,DRINK_MASTER_EXPANSION_B12,DRINK_MASTER_EXPANSION_B13,DRINK_MASTER_EXPANSION_B14,DRINK_MASTER_EXPANSION_B15,DRINK_MASTER_EXPANSION_B16,DRINK_MASTER_EXPANSION_B17,DRINK_MASTER_EXPANSION_B18,DRINK_MASTER_EXPANSION_B19,DRINK_MASTER_EXPANSION_B20,DRINK_MASTER_EXPANSION_B21,DRINK_MASTER_EXPANSION_B22,DRINK_MASTER_EXPANSION_B23,DRINK_MASTER_EXPANSION_B24,DRINK_MASTER_EXPANSION_B25,DRINK_MASTER_EXPANSION_B26,DRINK_MASTER_EXPANSION_B27,DRINK_MASTER_EXPANSION_B28,DRINK_MASTER_EXPANSION_B29,DRINK_MASTER_EXPANSION_B30,DRINK_MASTER_EXPANSION_B31,DRINK_MASTER_EXPANSION_B32,DRINK_MASTER_EXPANSION_B33,DRINK_MASTER_EXPANSION_B34,DRINK_MASTER_EXPANSION_B35,DRINK_MASTER_EXPANSION_B36,DRINK_MASTER_EXPANSION_B37,DRINK_MASTER_EXPANSION_B38,DRINK_MASTER_EXPANSION_B39,DRINK_MASTER_EXPANSION_B40];
const deferredKeys=new Set(DRINK_MASTER_EXPANSION_B40_EXCLUDED.filter(([,reason])=>reason==='quality-passed but deferred to next batch by 80-drink speed cap').map(([key])=>key));
export const DRINK_MASTER_EXPANSION_B41_DEFERRED=DRINK_MASTER_EXPANSION_B40_CANDIDATES.filter(d=>deferredKeys.has(d.masterKey));
export const DRINK_MASTER_EXPANSION_B41_CANDIDATES=[...DRINK_MASTER_EXPANSION_B41_DEFERRED,...DRINK_MASTER_EXPANSION_B41_MODERN_CANDIDATES];
...
const local=new Map();
export const DRINK_MASTER_EXPANSION_B41_EXCLUDED=[];
export const DRINK_MASTER_EXPANSION_B41=[];
for(const d of DRINK_MASTER_EXPANSION_B41_CANDIDATES){let reason='';const target=normalizeDrinkV19Key(d.masterKey);const names=[d.masterKey,d.nameJa,...(d.aliases||[])].map(normalizeDrinkV19Key);const selfSeen=new Set();for(const n of names){if(selfSeen.has(n)){reason='duplicate normalized self alias';break;}selfSeen.add(n);}if(!reason)for(const n of names){const hit=occupied.get(n);if(hit&&hit.target!==target){reason=`normalized name/alias collision with ${hit.source}`;break;}if(hit&&hit.target===target){reason=`existing normalized masterKey/alias in ${hit.source}`;break;}if(local.has(n)&&local.get(n)!==target){reason='local normalized alias collision';break;}if(local.has(n)&&local.get(n)===target){reason='duplicate normalized self alias';break;}}if(reason){DRINK_MASTER_EXPANSION_B41_EXCLUDED.push([d.masterKey,reason]);continue;}if(DRINK_MASTER_EXPANSION_B41.length>=14){DRINK_MASTER_EXPANSION_B41_EXCLUDED.push([d.masterKey,'quality-passed but target reached at 1500']);continue;}DRINK_MASTER_EXPANSION_B41.push(d);for(const n of names)local.set(n,target);}
export const DRINK_MASTER_EXPANSION_B41_ALIAS_ENTRIES=DRINK_MASTER_EXPANSION_B41.flatMap(d=>[[d.nameJa,d.masterKey],...(d.aliases||[]).map(a=>[a,d.masterKey])]);