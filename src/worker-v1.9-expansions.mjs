...
for (const batch of [...EXPANSION_BATCHES,B23_BATCH,B24_BATCH,B25_BATCH,B26_BATCH,B27_BATCH,B28_BATCH,B29_BATCH,B30_BATCH,B31_BATCH,B32_BATCH,B33_BATCH]) {
...
export { EXPANSION_BATCHES, B23_BATCH, B24_BATCH, B25_BATCH, B26_BATCH, B27_BATCH, B28_BATCH, B29_BATCH, B30_BATCH, B31_BATCH, B32_BATCH, B33_BATCH, expansionLookup, enrichExpansionResponse };
export default { async fetch(request, env, ctx) { const maintenance=Promise.allSettled(EXPANSION_BATCHES.map((batch)=>ensureBatch(env,batch)).concat(ensureBatch(env,B23_BATCH),ensureBatch(env,B24_BATCH),ensureBatch(env,B25_BATCH),ensureBatch(env,B26_BATCH),ensureBatch(env,B27_BATCH),ensureBatch(env,B28_BATCH),ensureBatch(env,B29_BATCH),ensureBatch(env,B30_BATCH),ensureBatch(env,B31_BATCH),ensureBatch(env,B32_BATCH),ensureBatch(env,B33_BATCH))); if (ctx?.waitUntil) ctx.waitUntil(maintenance); const response=await baseWorker.fetch(request,env,ctx); return enrichExpansionResponse(response,env); } };
