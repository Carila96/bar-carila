...
import { DRINK_MASTER_EXPANSION_B33_CANDIDATES,DRINK_MASTER_EXPANSION_B33_EVIDENCE_VERSION,DRINK_MASTER_EXPANSION_B33_EVALUATED_AT } from './drink-master-expansion-b33.mjs';
export { DRINK_MASTER_EXPANSION_B33_EVIDENCE_VERSION,DRINK_MASTER_EXPANSION_B33_EVALUATED_AT };
...
const semanticInitialAliases=new Set(['Harvard Cocktail','Hawaiian Cocktail','Ideal Cocktail','Honolulu Cocktail No. 1','Honolulu Cocktail No. 2'].map(normalizeDrinkV19Key));
const local=new Map();
export const DRINK_MASTER_EXPANSION_B33_EXCLUDED=[];
export const DRINK_MASTER_EXPANSION_B33=[];
for(const d of DRINK_MASTER_EXPANSION_B33_CANDIDATES){let reason='';const target=normalizeDrinkV19Key(d.masterKey);if(semanticInitialAliases.has(target))reason='semantic alias collision with initial-400 canonical drink';const names=[d.masterKey,d.nameJa,...(d.aliases||[])].map(normalizeDrinkV19Key);for(const n of names){const hit=occupied.get(n);if(hit&&hit.target!==target){reason=`normalized name/alias collision with ${hit.source}`;break;}if(hit&&hit.target===target){reason=`existing normalized masterKey/alias in ${hit.source}`;break;}if(local.has(n)&&local.get(n)!==target){reason='local normalized alias collision';break;}if(local.has(n)&&local.get(n)===target){reason='duplicate normalized self alias';break;}}if(reason){DRINK_MASTER_EXPANSION_B33_EXCLUDED.push([d.masterKey,reason]);continue;}DRINK_MASTER_EXPANSION_B33.push(d);for(const n of names)local.set(n,target);}
export const DRINK_MASTER_EXPANSION_B33_ALIAS_ENTRIES=DRINK_MASTER_EXPANSION_B33.flatMap(d=>[[d.nameJa,d.masterKey],...(d.aliases||[]).map(a=>[a,d.masterKey])]);
