...
import { DRINK_MASTER_EXPANSION_B27_CANDIDATES,DRINK_MASTER_EXPANSION_B27_EVIDENCE_VERSION,DRINK_MASTER_EXPANSION_B27_EVALUATED_AT } from './drink-master-expansion-b27.mjs';
export { DRINK_MASTER_EXPANSION_B27_EVIDENCE_VERSION,DRINK_MASTER_EXPANSION_B27_EVALUATED_AT };
const PRIOR=[DRINK_MASTER_EXPANSION_B01,DRINK_MASTER_EXPANSION_B02,DRINK_MASTER_EXPANSION_B03,DRINK_MASTER_EXPANSION_B04,DRINK_MASTER_EXPANSION_B05,DRINK_MASTER_EXPANSION_B06,DRINK_MASTER_EXPANSION_B07,DRINK_MASTER_EXPANSION_B08,DRINK_MASTER_EXPANSION_B09,DRINK_MASTER_EXPANSION_B10,DRINK_MASTER_EXPANSION_B11,DRINK_MASTER_EXPANSION_B12,DRINK_MASTER_EXPANSION_B13,DRINK_MASTER_EXPANSION_B14,DRINK_MASTER_EXPANSION_B15,DRINK_MASTER_EXPANSION_B16,DRINK_MASTER_EXPANSION_B17,DRINK_MASTER_EXPANSION_B18,DRINK_MASTER_EXPANSION_B19,DRINK_MASTER_EXPANSION_B20,DRINK_MASTER_EXPANSION_B21,DRINK_MASTER_EXPANSION_B22,DRINK_MASTER_EXPANSION_B23,DRINK_MASTER_EXPANSION_B24,DRINK_MASTER_EXPANSION_B25,DRINK_MASTER_EXPANSION_B26];
...
const local=new Map();
export const DRINK_MASTER_EXPANSION_B27_EXCLUDED=[];
export const DRINK_MASTER_EXPANSION_B27=[];
for(const d of DRINK_MASTER_EXPANSION_B27_CANDIDATES){let reason='';const target=normalizeDrinkV19Key(d.masterKey);const names=[d.masterKey,d.nameJa,...(d.aliases||[])].map(normalizeDrinkV19Key);for(const n of names){const hit=occupied.get(n);if(hit&&hit.target!==target){reason=`normalized name/alias collision with ${hit.source}`;break;}if(hit&&hit.target===target){reason=`existing normalized masterKey/alias in ${hit.source}`;break;}if(local.has(n)&&local.get(n)!==target){reason='local normalized alias collision';break;}if(local.has(n)&&local.get(n)===target){reason='duplicate normalized self alias';break;}}if(reason){DRINK_MASTER_EXPANSION_B27_EXCLUDED.push([d.masterKey,reason]);continue;}DRINK_MASTER_EXPANSION_B27.push(d);for(const n of names)local.set(n,target);}
export const DRINK_MASTER_EXPANSION_B27_ALIAS_ENTRIES=DRINK_MASTER_EXPANSION_B27.flatMap(d=>[[d.nameJa,d.masterKey],...(d.aliases||[]).map(a=>[a,d.masterKey])]);
