...
const semanticInitialAliases=new Set(['Third Degree Cocktail','Tipperary Cocktail No. 1','Tom and Jerry','Tuxedo Cocktail No. 1','Tuxedo Cocktail No. 2','Vermouth Cocktail','Ward Eight Cocktail','Waldorf Cocktail'].map(normalizeDrinkV19Key));
const qualityExcluded=new Map([
['Sweet Patootie Cocktail','primary scan fraction for orange juice is not sufficiently legible for high-confidence automated inclusion'],
['Three Stripes Cocktail','primary scan gin/vermouth fractions are not sufficiently legible for high-confidence automated inclusion'],
['Tinton Cocktail','primary scan port/apple-brandy fractions are not sufficiently legible for high-confidence automated inclusion'],
['Torpedo Cocktail','primary scan proportions are not sufficiently legible for high-confidence automated inclusion'],
['Trilby Cocktail No. 2','primary scan proportions for Parfait Amour/whisky/vermouth are not sufficiently legible for high-confidence automated inclusion'],
['Trocadero Cocktail','primary scan proportions are not sufficiently legible for high-confidence automated inclusion'],
['Tropical Cocktail','primary scan proportions are not sufficiently legible for high-confidence automated inclusion'],
['Twin Six Cocktail','primary scan gin/vermouth fractions are not sufficiently legible for high-confidence automated inclusion'],
['Vie Rose Cocktail','primary scan ingredient fractions are not sufficiently legible for high-confidence automated inclusion'],
['Volstead Cocktail','Hercules-based historical recipe is not sufficiently normalized for a stable modern representative recipe'],
['Vanderbilt Cocktail','primary scan brandy/cherry-brandy fractions are not sufficiently legible for high-confidence automated inclusion']
]);
const local=new Map();
export const DRINK_MASTER_EXPANSION_B38_EXCLUDED=[];
export const DRINK_MASTER_EXPANSION_B38=[];
for(const d of DRINK_MASTER_EXPANSION_B38_CANDIDATES){let reason=qualityExcluded.get(d.masterKey)||'';const target=normalizeDrinkV19Key(d.masterKey);if(!reason&&semanticInitialAliases.has(target))reason='semantic alias collision with established canonical drink';const names=[d.masterKey,d.nameJa,...(d.aliases||[])].map(normalizeDrinkV19Key);if(!reason){const selfSeen=new Set();for(const n of names){if(selfSeen.has(n)){reason='duplicate normalized self alias';break;}selfSeen.add(n);}}if(!reason)for(const n of names){const hit=occupied.get(n);if(hit&&hit.target!==target){reason=`normalized name/alias collision with ${hit.source}`;break;}if(hit&&hit.target===target){reason=`existing normalized masterKey/alias in ${hit.source}`;break;}if(local.has(n)&&local.get(n)!==target){reason='local normalized alias collision';break;}if(local.has(n)&&local.get(n)===target){reason='duplicate normalized self alias';break;}}if(reason){DRINK_MASTER_EXPANSION_B38_EXCLUDED.push([d.masterKey,reason]);continue;}DRINK_MASTER_EXPANSION_B38.push(d);for(const n of names)local.set(n,target);}
export const DRINK_MASTER_EXPANSION_B38_ALIAS_ENTRIES=DRINK_MASTER_EXPANSION_B38.flatMap(d=>[[d.nameJa,d.masterKey],...(d.aliases||[]).map(a=>[a,d.masterKey])]);
