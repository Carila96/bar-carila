...
const semanticInitialAliases=new Set(['Sazerac Cocktail','Sidecar Cocktail','Silver Bullet Cocktail','South Side Cocktail','Stinger Cocktail','Stone Fence Cocktail'].map(normalizeDrinkV19Key));
const qualityExcluded=new Map([
['Savoy Hotel Special No. 1','primary recipe proportions not sufficiently certain for automated inclusion'],
['Sherry and Egg','historical service method is insufficiently normalized for a stable representative recipe'],
['Sunset Cocktail','primary recipe quantities are too ambiguous for high-confidence automated inclusion'],
['Swazi Freeze','primary recipe fractions are insufficiently legible for high-confidence automated inclusion']
]);
const local=new Map();
export const DRINK_MASTER_EXPANSION_B37_EXCLUDED=[];
export const DRINK_MASTER_EXPANSION_B37=[];
for(const d of DRINK_MASTER_EXPANSION_B37_CANDIDATES){let reason=qualityExcluded.get(d.masterKey)||'';const target=normalizeDrinkV19Key(d.masterKey);if(!reason&&semanticInitialAliases.has(target))reason='semantic alias collision with established canonical drink';const names=[d.masterKey,d.nameJa,...(d.aliases||[])].map(normalizeDrinkV19Key);if(!reason){const selfSeen=new Set();for(const n of names){if(selfSeen.has(n)){reason='duplicate normalized self alias';break;}selfSeen.add(n);}}if(!reason)for(const n of names){const hit=occupied.get(n);if(hit&&hit.target!==target){reason=`normalized name/alias collision with ${hit.source}`;break;}if(hit&&hit.target===target){reason=`existing normalized masterKey/alias in ${hit.source}`;break;}if(local.has(n)&&local.get(n)!==target){reason='local normalized alias collision';break;}if(local.has(n)&&local.get(n)===target){reason='duplicate normalized self alias';break;}}if(reason){DRINK_MASTER_EXPANSION_B37_EXCLUDED.push([d.masterKey,reason]);continue;}DRINK_MASTER_EXPANSION_B37.push(d);for(const n of names)local.set(n,target);}
export const DRINK_MASTER_EXPANSION_B37_ALIAS_ENTRIES=DRINK_MASTER_EXPANSION_B37.flatMap(d=>[[d.nameJa,d.masterKey],...(d.aliases||[]).map(a=>[a,d.masterKey])]);
