...
import { DRINK_MASTER_EXPANSION_B02, DRINK_MASTER_EXPANSION_B02_SEED_ROWS, DRINK_MASTER_EXPANSION_B02_ALIAS_ENTRIES, DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B02_EVALUATED_AT } from './drink-master-expansion-b02.mjs';
...
import { DRINK_MASTER_EXPANSION_B07, DRINK_MASTER_EXPANSION_B07_SEED_ROWS, DRINK_MASTER_EXPANSION_B07_ALIAS_ENTRIES, DRINK_MASTER_EXPANSION_B07_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B07_EVALUATED_AT } from './drink-master-expansion-b07.mjs';
import { DRINK_MASTER_EXPANSION_B08, DRINK_MASTER_EXPANSION_B08_SEED_ROWS, DRINK_MASTER_EXPANSION_B08_ALIAS_ENTRIES, DRINK_MASTER_EXPANSION_B08_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B08_EVALUATED_AT } from './drink-master-expansion-b08.mjs';

const EVIDENCE_VERSION = 'jp-rarity-v1.9';
const EVALUATED_AT = '2026-09-05';
const V19_BY_KEY = new Map([...JP_RARITY_V19_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B01_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B02_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B03_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B04_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B05_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B06_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B07_SEED_ROWS, ...DRINK_MASTER_EXPANSION_B08_SEED_ROWS].map((row) => [normalizeDrinkV19Key(row[0]), row]));
...
for (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B03_ALIAS_ENTRIES) {
  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));
}
for (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B04_ALIAS_ENTRIES) {
  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));
}
...
for (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B07_ALIAS_ENTRIES) {
  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));
}
for (const [alias, masterKey] of DRINK_MASTER_EXPANSION_B08_ALIAS_ENTRIES) {
  LOOKUP_ALIASES.set(normalizeDrinkV19Key(alias), normalizeDrinkV19Key(masterKey));
}
const ACCEPTED_EVIDENCE_VERSIONS = new Set([EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B01_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B04_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B05_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B06_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B07_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B08_EVIDENCE_VERSION]);
let v19Ready;
...
async function hasExpansionB02Seed(env) {
  if (!env?.DRINK_DB) return false;
  try {
    const row = await env.DRINK_DB.prepare(`SELECT COUNT(*) AS count FROM drinks WHERE evidence_version = ?`)
      .bind(DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION).first();
    return Number(row?.count || 0) >= DRINK_MASTER_EXPANSION_B02.length;
  } catch {
    return false;
  }
}

async function seedExpansionB02(env) {
  if (!env?.DRINK_DB) return;
  await ensureV19Tables(env);
  const statements = DRINK_MASTER_EXPANSION_B02.map((drink) => {
    const key = normalizeDrinkV19Key(drink.masterKey);
    return env.DRINK_DB.prepare(`INSERT INTO drinks (
      canonical_key, name_ja, name_en, category, base_spirit, drink_kind,
      japan_availability_score, japan_rarity_score, japan_rarity_label, japan_rarity_confidence,
    ...
    ON CONFLICT(canonical_key) DO UPDATE SET
      ...
      .bind(key, drink.nameJa, drink.masterKey, drink.category, drink.baseSpirit, drink.drinkKind,
        drink.availability, drink.rarity, drink.rarityLabel, drink.confidence, drink.rarityReason,
        DRINK_MASTER_EXPANSION_B02_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B02_EVALUATED_AT,
        drink.shortDescription, drink.orderHint);
  });
  if (statements.length) await env.DRINK_DB.batch(statements);
  for (const drink of DRINK_MASTER_EXPANSION_B02) {
    ...
    for (const evidence of drink.evidence) {
      ...
      await env.DRINK_DB.prepare(`INSERT INTO drink_evidence (
        ...
        .bind(row.id, evidence.type, evidence.title, evidence.url, evidence.note, DRINK_MASTER_EXPANSION_B02_EVALUATED_AT, 'supporting').run();
    }
  }
}


async function hasExpansionB03Seed(env) {
  if (!env?.DRINK_DB) return false;
  try {
    const row = await env.DRINK_DB.prepare(`SELECT COUNT(*) AS count FROM drinks WHERE evidence_version = ?`)
      .bind(DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION).first();
    return Number(row?.count || 0) >= DRINK_MASTER_EXPANSION_B03.length;
  } catch {
    return false;
  }
}

async function seedExpansionB03(env) {
  if (!env?.DRINK_DB) return;
  await ensureV19Tables(env);
  const statements = DRINK_MASTER_EXPANSION_B03.map((drink) => {
    const key = normalizeDrinkV19Key(drink.masterKey);
    return env.DRINK_DB.prepare(`INSERT INTO drinks (
      canonical_key, name_ja, name_en, category, base_spirit, drink_kind,
      japan_availability_score, japan_rarity_score, japan_rarity_label, japan_rarity_confidence,
    ...
    ON CONFLICT(canonical_key) DO UPDATE SET
      ...
      .bind(key, drink.nameJa, drink.masterKey, drink.category, drink.baseSpirit, drink.drinkKind,
        drink.availability, drink.rarity, drink.rarityLabel, drink.confidence, drink.rarityReason,
        DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B03_EVALUATED_AT,
        drink.shortDescription, drink.orderHint);
  });
  if (statements.length) await env.DRINK_DB.batch(statements);
  for (const drink of DRINK_MASTER_EXPANSION_B03) {
    const key = normalizeDrinkV19Key(drink.masterKey);
    const row = await env.DRINK_DB.prepare(`SELECT id FROM drinks WHERE canonical_key = ? LIMIT 1`).bind(key).first();
    if (!row?.id) continue;
    const aliasStatements = [drink.nameJa, ...drink.aliases].map((alias) => env.DRINK_DB.prepare(`INSERT INTO drink_aliases (
    ...
    for (const evidence of drink.evidence) {
      ...
      await env.DRINK_DB.prepare(`INSERT INTO drink_evidence (
        ...
        .bind(row.id, evidence.type, evidence.title, evidence.url, evidence.note, DRINK_MASTER_EXPANSION_B03_EVALUATED_AT, 'supporting').run();
    }
  }
}

async function hasExpansionB04Seed(env) {
  if (!env?.DRINK_DB) return false;
  try {
    const row = await env.DRINK_DB.prepare(`SELECT COUNT(*) AS count FROM drinks WHERE evidence_version = ?`)
...
async function hasExpansionB05Seed(env) {
  if (!env?.DRINK_DB) return false;
  try {
    const row = await env.DRINK_DB.prepare(`SELECT COUNT(*) AS count FROM drinks WHERE evidence_version = ?`)
      .bind(DRINK_MASTER_EXPANSION_B05_EVIDENCE_VERSION).first();
    return Number(row?.count || 0) >= DRINK_MASTER_EXPANSION_B05.length;
  } catch {
    return false;
  }
}

async function seedExpansionB05(env) {
  if (!env?.DRINK_DB) return;
  await ensureV19Tables(env);
  const statements = DRINK_MASTER_EXPANSION_B05.map((drink) => {
    const key = normalizeDrinkV19Key(drink.masterKey);
    return env.DRINK_DB.prepare(`INSERT INTO drinks (
      canonical_key, name_ja, name_en, category, base_spirit, drink_kind,
      japan_availability_score, japan_rarity_score, japan_rarity_label, japan_rarity_confidence,
    ...
    ON CONFLICT(canonical_key) DO UPDATE SET
      ...
      .bind(key, drink.nameJa, drink.masterKey, drink.category, drink.baseSpirit, drink.drinkKind,
        drink.availability, drink.rarity, drink.rarityLabel, drink.confidence, drink.rarityReason,
        DRINK_MASTER_EXPANSION_B05_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B05_EVALUATED_AT,
        drink.shortDescription, drink.orderHint);
  });
  if (statements.length) await env.DRINK_DB.batch(statements);
  for (const drink of DRINK_MASTER_EXPANSION_B05) {
    ...
    for (const evidence of drink.evidence) {
      ...
      await env.DRINK_DB.prepare(`INSERT INTO drink_evidence (
        ...
        .bind(row.id, evidence.type, evidence.title, evidence.url, evidence.note, DRINK_MASTER_EXPANSION_B05_EVALUATED_AT, 'supporting').run();
    }
  }
}
...
async function hasExpansionB07Seed(env) {
  if (!env?.DRINK_DB) return false;
  try {
    const row = await env.DRINK_DB.prepare(`SELECT COUNT(*) AS count FROM drinks WHERE evidence_version = ?`)
      .bind(DRINK_MASTER_EXPANSION_B07_EVIDENCE_VERSION).first();
    return Number(row?.count || 0) >= DRINK_MASTER_EXPANSION_B07.length;
  } catch {
    return false;
  }
}

async function seedExpansionB07(env) {
  if (!env?.DRINK_DB) return;
  await ensureV19Tables(env);
  const statements = DRINK_MASTER_EXPANSION_B07.map((drink) => {
    const key = normalizeDrinkV19Key(drink.masterKey);
    return env.DRINK_DB.prepare(`INSERT INTO drinks (
      canonical_key, name_ja, name_en, category, base_spirit, drink_kind,
      japan_availability_score, japan_rarity_score, japan_rarity_label, japan_rarity_confidence,
    ...
    ON CONFLICT(canonical_key) DO UPDATE SET
      ...
      .bind(key, drink.nameJa, drink.masterKey, drink.category, drink.baseSpirit, drink.drinkKind,
        drink.availability, drink.rarity, drink.rarityLabel, drink.confidence, drink.rarityReason,
        DRINK_MASTER_EXPANSION_B07_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B07_EVALUATED_AT,
        drink.shortDescription, drink.orderHint);
  });
  if (statements.length) await env.DRINK_DB.batch(statements);
  for (const drink of DRINK_MASTER_EXPANSION_B07) {
    const key = normalizeDrinkV19Key(drink.masterKey);
    const row = await env.DRINK_DB.prepare(`SELECT id FROM drinks WHERE canonical_key = ? LIMIT 1`).bind(key).first();
    if (!row?.id) continue;
    const aliasStatements = [drink.nameJa, ...drink.aliases].map((alias) => env.DRINK_DB.prepare(`INSERT INTO drink_aliases (
    ...
    for (const evidence of drink.evidence) {
      ...
      await env.DRINK_DB.prepare(`INSERT INTO drink_evidence (
        ...
        .bind(row.id, evidence.type, evidence.title, evidence.url, evidence.note, DRINK_MASTER_EXPANSION_B07_EVALUATED_AT, 'supporting').run();
    }
  }
}

async function hasExpansionB08Seed(env) {
  if (!env?.DRINK_DB) return false;
  try {
    const row = await env.DRINK_DB.prepare(`SELECT COUNT(*) AS count FROM drinks WHERE evidence_version = ?`)
      .bind(DRINK_MASTER_EXPANSION_B08_EVIDENCE_VERSION).first();
    return Number(row?.count || 0) >= DRINK_MASTER_EXPANSION_B08.length;
  } catch {
    return false;
  }
}

async function seedExpansionB08(env) {
  if (!env?.DRINK_DB) return;
  await ensureV19Tables(env);
  const statements = DRINK_MASTER_EXPANSION_B08.map((drink) => {
    const key = normalizeDrinkV19Key(drink.masterKey);
    return env.DRINK_DB.prepare(`INSERT INTO drinks (
      canonical_key, name_ja, name_en, category, base_spirit, drink_kind,
      japan_availability_score, japan_rarity_score, japan_rarity_label, japan_rarity_confidence,
    ...
    ON CONFLICT(canonical_key) DO UPDATE SET
      ...
      .bind(key, drink.nameJa, drink.masterKey, drink.category, drink.baseSpirit, drink.drinkKind,
        drink.availability, drink.rarity, drink.rarityLabel, drink.confidence, drink.rarityReason,
        DRINK_MASTER_EXPANSION_B08_EVIDENCE_VERSION, DRINK_MASTER_EXPANSION_B08_EVALUATED_AT,
        drink.shortDescription, drink.orderHint);
  });
  if (statements.length) await env.DRINK_DB.batch(statements);
  for (const drink of DRINK_MASTER_EXPANSION_B08) {
    const key = normalizeDrinkV19Key(drink.masterKey);
    const row = await env.DRINK_DB.prepare(`SELECT id FROM drinks WHERE canonical_key = ? LIMIT 1`).bind(key).first();
    if (!row?.id) continue;
    const aliasStatements = [drink.nameJa, ...drink.aliases].map((alias) => env.DRINK_DB.prepare(`INSERT INTO drink_aliases (
    ...
    for (const evidence of drink.evidence) {
      ...
      await env.DRINK_DB.prepare(`INSERT INTO drink_evidence (
        ...
        .bind(row.id, evidence.type, evidence.title, evidence.url, evidence.note, DRINK_MASTER_EXPANSION_B08_EVALUATED_AT, 'supporting').run();
    }
  }
}
...
async function seedV19IfNeeded(env) {
  ...
  if (!(await hasExpansionB02Seed(env))) await seedExpansionB02(env);
...
export { JP_RARITY_V19_SEED_ROWS as V19_ROWS, rarityLabel, rarityReason, canonicalLookupKey, enrichV19Response, hasCurrentV19Seed, hasExpansionB01Seed, hasExpansionB02Seed, hasExpansionB03Seed, hasExpansionB04Seed, hasExpansionB05Seed, hasExpansionB06Seed, hasExpansionB07Seed, hasExpansionB08Seed };
