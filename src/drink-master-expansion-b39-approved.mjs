...
const local=new Map();
export const DRINK_MASTER_EXPANSION_B39_EXCLUDED=[];
export const DRINK_MASTER_EXPANSION_B39=[];
for(const d of DRINK_MASTER_EXPANSION_B39_CANDIDATES){let reason='';const target=normalizeDrinkV19Key(d.masterKey);const names=[d.masterKey,d.nameJa,...(d.aliases||[])].map(normalizeDrinkV19Key);const selfSeen=new Set();for(const n of names){if(selfSeen.has(n)){reason='duplicate normalized self alias';break;}selfSeen.add(n);}if(!reason)for(const n of names){const hit=occupied.get(n);if(hit&&hit.target!==target){reason=`normalized name/alias collision with ${hit.source}`;break;}if(hit&&hit.target===target){reason=`existing normalized masterKey/alias in ${hit.source}`;break;}if(local.has(n)&&local.get(n)!==target){reason='local normalized alias collision';break;}if(local.has(n)&&local.get(n)===target){reason='duplicate normalized self alias';break;}}if(reason){DRINK_MASTER_EXPANSION_B39_EXCLUDED.push([d.masterKey,reason]);continue;}DRINK_MASTER_EXPANSION_B39.push(d);for(const n of names)local.set(n,target);}
export const DRINK_MASTER_EXPANSION_B39_ALIAS_ENTRIES=DRINK_MASTER_EXPANSION_B39.flatMap(d=>[[d.nameJa,d.masterKey],...(d.aliases||[]).map(a=>[a,d.masterKey])]);
