...
import {
  ...
  DRINK_MASTER_EXPANSION_B03_ALIAS_ENTRIES,
  DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION,
} from '../src/drink-master-expansion-b03.mjs';

test('expansion batch 03 is additive and does not duplicate the initial 400 or prior batches', () => {
  assert.equal(DRINK_MASTER_EXPANSION_B03.length, 5);
  ...
  const keys = DRINK_MASTER_EXPANSION_B03.map((drink) => normalizeDrinkV19Key(drink.masterKey));
  assert.equal(new Set(keys).size, keys.length);
  for (const key of keys) assert.equal(occupied.has(key), false, `duplicate known master key: ${key}`);
});

test('expansion batch 03 has complete normalized aliases, recipes, rarity and evidence', () => {
  ...
  assert.equal(DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION, 'jp-rarity-expansion-2026-09-08-b03');
});

test('runtime is wired to seed and read batch 03', async () => {
  const source = await import('node:fs/promises').then(({ readFile }) => readFile(new URL('../src/worker-v1.9.mjs', import.meta.url), 'utf8'));
  assert.match(source, /DRINK_MASTER_EXPANSION_B03/);
  assert.match(source, /seedExpansionB03/);
  assert.match(source, /hasExpansionB03Seed/);
  assert.match(source, /DRINK_MASTER_EXPANSION_B03_EVIDENCE_VERSION/);
});
